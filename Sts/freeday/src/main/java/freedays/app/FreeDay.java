...
/**
 * Abstract class representing a Request object for the FreeDay context.
 * 
 * @author fmacicasan
 * 
 */
...
public abstract class FreeDay {
	...
	/**
	 * Returns the current approver based on the given user and the current
	 * strategy.
	 * 
	 * @param user
	 *            the owner of the request associated with this request object
	...
	/**
	 * Describes the states in which a FreeDay object can be. These states will
	 * mark the status of a FreeDay. Such a status is synchronized with the
	 * status of a Request.
	 * 
	 * @author fmacicasan
	 * @see RequestStatus
	...
	public enum FreeDayStatus {
		...
		/**
		 * The FreeDay is associated with an approved request but is waiting for
		 * a complementary request to complete successfully.
		 */
		WAITING,

		/**
		 * The FreeDays is associated with an request that was approved and, if
		 * it is the case, its associated with a complementary request.
		 */
		FINALIZE_SUCCESS,

		/**
		 * The FreeDay is associated with a request that terminated with
		 * failure. It was either denied or canceled.
		 */
		COMPLETED_FAILURE;
	...
	/**
	 * Verifies weather or not there still is a successors in the approval chain
	 * for the current approval strategy. If there still is a successor, it
	 * becomes the current approval strategy otherwise the approval strategy is
	 * set to null.
	 * 
	...
	public boolean isReportable() {
		return this.getDate().before(Calendar.getInstance())
				&& (this.getStatus() == FreeDayStatus.WAITING || this.getStatus() == FreeDayStatus.FINALIZE_SUCCESS);
	}
	...
	/**
	 * Returns a calendar instance registered by a child as being the primary
	 * date of the FreeDay.
	 * 
	...
	/**
	 * Gets the free day status of the FreeDay when the associated Request is
	 * approved.
	 * 
	...
	/**
	 * Performs custom child initialization needed upon creation of child
	 * instances but not supported via the FreeDay creation process. Any new
	 * member a child defines should be initialized using this method.
	 * 
	 * @param fdr
	 *            the request holding the initialization information
	 */
	protected abstract void initialize(FreeDayRequest fdr);

	/**
	 * Performs custom child finalization when a Request fails. Any specialized
	 * fail behavior a child needs to implement should be implemented using this
	 * method.
	 */
	...
	/**
	 * Sets the final approve status based on child's implementation.
	 */
	...
	public static FreeDay createPersistentFreeDay(FreeDayRequest fdr) {
		if (fdr == null)
			throw new IllegalArgumentException("The FreeDayRequest argument is required");
	...
	/**
	 * Returns a list of FreeDayUserList containing the approved free days for
	 * each FDUser in the database.
	 * 
	 * @return the list
	 * @see FDUser
	 * @see FreeDayUserList
	...
	/**
	 * Generates a list of all the granted free days for a given FDUser based on
	 * his username.
	 * 
	 * @param username
	 * @return
	...
	public static List<FreeDay> getAllGrantedFreeDayByUsername(String username) {
		...
		TypedQuery<FreeDay> q = em.createQuery(
				"SELECT o FROM FreeDay o, Request r WHERE r.appreguser.regularUser.username = :username AND r.requestable = o AND o.status IN :approveList ",
				FreeDay.class);
	...
	/**
	 * Returns a list of FreeDayUserList containing the approved vacations for
	 * each FDUser in the database.
	 * 
	 * @return the list
	 * @see FDUser
	...
	public static List<FreeDayUserList> getAllUserVacations(Calendar start, Calendar end) {
		if (start == null)
			throw new IllegalArgumentException("the start argument is required");
		if (end == null)
			throw new IllegalArgumentException("the end argument is required");
		if (start.after(end))
			throw new IllegalArgumentException("the start should be before the end");
		List<FreeDayUserList> fdrl = new ArrayList<FreeDayUserList>();
		List<FDUser> fdul = FDUser.findAllFDUsers();
		for (FDUser fdUser : fdul) {
	...
	public static List<FreeDay> getAllNotFailedRequestsByUsername(String username) {
		...
		TypedQuery<FreeDay> q = em
				.createQuery(
						"SELECT o FROM FreeDay o, Request r WHERE r.appreguser.regularUser.username = :username AND r.requestable = o AND o.status != :completedfailure",
						FreeDay.class);
		q.setParameter("username", username);
	...
	public boolean verifyUniqueness(Calendar date) {
		if (date == null)
			throw new IllegalArgumentException("The date argument is required");
		return this.getDate().equals(date);
	}

	/**
	 * Counts all the type C free days that are under approval or already
	 * approved associated with a request made by the provided FDUser
	 * 
	 * @param fdUser
	 * @return
	 */
	public static long countAllNotFailedTypeCRequestsByFDUser(FDUser fdUser) {
		if (fdUser == null)
			throw new IllegalArgumentException("The fdUser argument is required");
		EntityManager em = FreeDay.entityManager();
		TypedQuery<Long> q = em.createQuery(
				"SELECT COUNT(o) FROM FreeDayC o, Request r WHERE r.appreguser = :fdUser AND r.requestable = o AND o.status != :completedfailure", Long.class);
