...
/**
 * 
 * Defines an interval of free days
 * 
 * 
 * @author osuciu
 *
...
@RooToString
@RooEntity
public abstract class FreeDayInterval extends FreeDay {

	static final Logger logger = LoggerFactory.getLogger(FreeDayInterval.class);

	// @NotNull
	// @Future removed to solve IN-105 request creation is still restricted from
	// the wrapper but in the backend
	// such entities should be matched so they will be processed some time in
	// the future with an already specified
	// date
	...
	/**
	 * Initialize a FreeDayInterval based on the request object
	 */
	@Override
	protected void initialize(FreeDayRequest fdr) {
		// if(!(fdr instanceof FreeDayRequestVacation))throw new
		// IllegalArgumentException("The fdr argument must be of type FreeDayRequestVacation");
		if (fdr instanceof FreeDayRequestInterval) {
			// initialize a multiple days vacation
			FreeDayRequestInterval fdrv = (FreeDayRequestInterval) fdr;
	...
	public static List<FreeDayVacation> getAllGrantedVacationsByUsername(String username) {
		...
		TypedQuery<FreeDayVacation> q = em
				.createQuery(
						"SELECT o FROM FreeDayVacation o, Request r WHERE r.appreguser.regularUser.username = :username AND r.requestable = o AND r.status = :approveStatus ",
						FreeDayVacation.class);
	 ...
	 * made by a FDUser associated to a regular user identified by the provided
	 * username
	 * 
	 * @param username
	 *            \
	...
	public static Long sumAllVacationSpansByUsername(String username, Integer year) {
		...
		TypedQuery<Long> q = em
				.createQuery(
						"SELECT SUM(o.span - o.number) FROM FreeDayVacation o, Request r WHERE r.appreguser.regularUser.username = :username AND r.requestable = o AND o.status != :completedfailure AND o.year = :year",
						Long.class);
		q.setParameter("username", username);
	 ...
	 * date for a new free day request.
	 */
	@Override
	public boolean verifyUniqueness(Calendar date) {
		if (date == null)
			throw new IllegalArgumentException("The date argument is required");
		return (this.getDate().before(date) && this.getEnd().after(date)) || this.getDate().equals(date) || this.getEnd().equals(date);
	}
	...
	public Calendar getEnd() {
		// logger.info(String.format("The current date for %s and span %d -> %s",DateUtils.printShortDate(this.getDate()),this.getSpan(),DateUtils.printShortDate(DateUtils.dateAddRomanianBusinessDay(this.getDate(),
		// this.getSpan()))));
		return DateUtils.dateAddRomanianBusinessDay(this.getDate(), this.getSpan());
	}

	/**
	 * Counts all the free days that are under approval or already approved that
	 * are associated with requests made by a FDUser associated with a
	...
	public static Long countAllNotFailedRequestsByUsername(String username, Integer year) {
		...
		TypedQuery<Long> q = em
				.createQuery(
						"SELECT COUNT(o) FROM FreeDayVacation o, Request r WHERE r.appreguser.regularUser.username = :username AND r.requestable = o AND o.status != :completedfailure AND o.year = :year",
						Long.class);
		q.setParameter("username", username);
		q.setParameter("completedfailure", FreeDayStatus.COMPLETED_FAILURE);
		q.setParameter("year", year);
	...
	public String getDateReport() {
		...
		if (this.getSpan() == 0) {
			...
			sb.append("From ").append(diffYear ? DateUtils.printShortDateYear(this.getDate()) : DateUtils.printShortDate(this.getDate()));
			...
			sb.append(" To ").append(diffYear ? DateUtils.printShortDateYear(this.getEnd()) : DateUtils.printShortDate(this.getEnd()));
			// DateUtils.dateAddRomanianBusinessDay(this.getDate(),
			// this.getSpan()))
		}
