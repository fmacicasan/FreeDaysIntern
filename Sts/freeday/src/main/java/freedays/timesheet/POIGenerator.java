...
public class POIGenerator implements TimesheetGenerator {
	...
	private static String location = PropertiesUtil.getProperty("timesheet.generation.location");
	// "..\\timesheets\\"
	...
	CellStyle datestyle;
	/**
	 * Array of PhaseLabors specific to a certain week(these constitute the
	 * table header
	 */
	...
	private Pattern getPatternForDay(Calendar x) {
		...
		for (int i = 0; i < schedEmp.size(); i++) {
			// System.out.println("schedEmp start"+DateUtils.printShortDateYear(schedEmp.get(i).getStartDate())
			// +" end "+DateUtils.printShortDateYear(schedEmp.get(i).getEndDate()));

			if ((x.compareTo(schedEmp.get(i).getStartDate()) >= 0)
					&& (x.compareTo(schedEmp.get(i).getEndDate()) <= 0)) {
	 ...
	 * @return
	 */
	private ArrayList<PhaseLabor> getListOfDistinctPhaseLabor(
		...
		for (int i = 0; i < pList.size(); i++) {
			...
			for (int j = 0; j < lph.size(); j++) {
				...
				for (int k = 0; k < PhLArr.size(); k++) {
					if (PhLArr.get(k).getLaborbilling().getId()
							.equals(currentPhL.getLaborbilling().getId()))
						if (PhLArr.get(k).getPhase().getId()
								.equals(currentPhL.getPhase().getId()))
							if (PhLArr.get(k).getProject().getId()
									.equals(currentPhL.getProject().getId())) {
								ok = 0;
							}
				}
				if (ok == 1)
	...
	private ArrayList<Pattern> getListOfPatterns(Calendar weekEnd,
	 ...
	 * @param bigSum
	 */
	private void generateTableFooter(int startingRow, int startingDay,
			int endingDay, Calendar weekEnd, float bigSum) {
		Row z = sheet1.createRow(startingRow + PhLArray.size() + 1);
		CellRangeAddress region = new CellRangeAddress(startingRow
				+ PhLArray.size() + 1, startingRow + PhLArray.size() + 1, 0, 3);
	...
	//
	// }
	private Float generateTableRow(Integer startingRow, int i,
			Integer startingDay, Integer endingDay, Calendar weekEnd) {
		...
		for (int j = startingDay - 1; j < endingDay; j++) {
			...
			if (mDay.before(hireDate)) {
				...
				Pattern mPattern = getPatternForDay(mDay);
				PhaseLabor phLX = mPattern.getPhaseLabor(PhLArray.get(i - 1)
						.getLaborbilling(), PhLArray.get(i - 1).getPhase(),
						PhLArray.get(i - 1).getProject());
	 ...
	 * @param startingRow
	 * @param startingDay
	 *            1-5 as Monday-Friday
	 * @param endingDay
	 *            1-5 as Monday-Friday
	 * @param weekEnd
	 *            last day of week
	 */
	private void generateWeek(Integer startingRow, Integer startingDay,
			Integer endingDay, Calendar weekEnd) {
		...
		Row WE = sheet1.createRow(startingRow + 1);// TODO:Changed by fmacicasan
													// 12:56 30.09.2011
		...
		if ((cloneCurrent.get(Calendar.MONTH) != weekEnd.get(Calendar.MONTH))) {
			if ((endingDay != 5)
					|| ((endingDay == 5) && (((weekEnd
							.get(Calendar.DAY_OF_MONTH)) == 1) || ((weekEnd
							.get(Calendar.DAY_OF_MONTH)) == 2)))) {
				daysInMonth = cloneCurrent
						.getActualMaximum(Calendar.DAY_OF_MONTH);
				Integer currentDay = cloneCurrent.get(Calendar.DAY_OF_MONTH);
				cloneCurrent.add(Calendar.DAY_OF_MONTH, daysInMonth
						- currentDay);
			} else
		...
		for (int i = 0; i <= PhLArray.size(); i++) {
			if (i == 0) {
				...
				bigSum += generateTableRow(startingRow, i, startingDay,
						endingDay, weekEnd);
			}
		}
		generateTableFooter(startingRow, startingDay, endingDay, weekEnd,
				bigSum);
	}

	private int howManyDays(Calendar t) {
		int K = t.getActualMaximum(Calendar.DAY_OF_MONTH)
	...
	private void unionScheduleVacation() {
		...
		List<FreeDayAbstraction> lFreeDays = FreeDayAbstraction
				.getAllGrantedFreeDayAbstractions(pEmp.getRegularUser()
						.getUsername());
		for (int i = 0; i < lFreeDays.size(); i++) {
			...
			for (int j = 0; j < schedEmp.size(); j++) {
				/*
				 * schedEmpSegment |-------------------| overriding Segment
				 * |-------------------------------|
				 */
				if (schedEmp.get(j).getStartDate().after(start)
						&& schedEmp.get(j).getEndDate().before(end)) {
					continue;
				} else
				/*
				 * schedEmpSegment |-------------------| overriding Segment
				 * |-----------------|
				 */
				if (((schedEmp.get(j).getStartDate().after(start) || (schedEmp
						.get(j).getStartDate().equals(start))) && (end
						.after(schedEmp.get(j).getStartDate())))) {
				 ...
				 * |-------|
				 */
				if (start.after(schedEmp.get(j).getStartDate())
						&& (end.before(schedEmp.get(j).getEndDate()))) {
				 ...
				 * |--------------|
				 */
				if ((start.before(schedEmp.get(j).getEndDate()) && (end
						.after(schedEmp.get(j).getEndDate())))
						|| end.equals(schedEmp.get(j).getEndDate())) {
	...
	public void generateDocFooter(int genMonth, int genYear) {
		...
		CellRangeAddress regionFooterSignature = new CellRangeAddress(noOfRows,
				noOfRows, 0, 2);
		CellRangeAddress regionFooterApprovedBy = new CellRangeAddress(
				noOfRows + 1, noOfRows + 1, 0, 2);
		CellRangeAddress regionFooterSignatureEmp = new CellRangeAddress(
				noOfRows, noOfRows, 3, 4);
		CellRangeAddress regionFooterSignatureAppBy = new CellRangeAddress(
				noOfRows + 1, noOfRows + 1, 3, 4);
	...
	public File generateDoc(String workbookname, int genmonth, int genyear) {
		...
		sheet1.getPrintSetup().setLandscape(true);// TODO:Changed by fmacicasan
													// 12:56 30.09.2011
		generateDocHeader(sheet1,genmonth);
		noOfRows = 5;
		Calendar TimeSheetCalendar = handleFirstWeeksOfMonth(genyear, genmonth);
		Integer noOfDays = TimeSheetCalendar
				.getActualMaximum(Calendar.DAY_OF_MONTH);
		while (TimeSheetCalendar.get(Calendar.DAY_OF_MONTH) + 7 <= noOfDays) {
			TimeSheetCalendar.add(Calendar.DAY_OF_MONTH, 7);
			Calendar auxWE = (Calendar) TimeSheetCalendar.clone();
			/*
			 * Because TimesheetCalendar is always set on monday I add 6
			 * DAY_OF_MONTH to get the next sunday
			 */
			auxWE.add(Calendar.DAY_OF_MONTH, 6);
